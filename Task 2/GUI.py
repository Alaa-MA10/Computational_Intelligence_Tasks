...
Label(mainWin, text="Learning Rate", font=('Bahnschrift', 11)).place(x=30, y=280)
LearningRate_txt = Entry(mainWin)
LearningRate_txt.place(x=155, y=280)

# Epochs [Label & TextBox]
Label(mainWin, text="Epochs", font=("Bahnschrift", 11)).place(x=80, y=315)
epochs_txt = Entry(mainWin)
epochs_txt.place(x=155, y=315)

# MSE threshold  [Label & TextBox]
Label(mainWin, text="MSE threshold ", font=("Bahnschrift", 11)).place(x=30, y=350)
MSE_txt = Entry(mainWin)
MSE_txt.place(x=155, y=350)

Bias_Selected = IntVar()
Checkbutton(mainWin, text="Bias", variable=Bias_Selected, padx=10, font=('Arial', 11, 'bold')).place(x=145, y=390)


def Train():
    n, epochs, learning_rate = check_data()
    Mse_Therehold = float(MSE_txt.get())
    if n == 1:
        NN.Learn(get_selected_item(Feature_List), get_selected_item(Classes_List),
                           learning_rate, int(epochs), Bias_Selected.get(),Mse_Therehold)
